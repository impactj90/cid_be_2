...
use crate::{
    ...
    constants,
    error::ServiceError,
    models::{customer::CustomerDTO, response::ResponseBody},
...
pub async fn insert(
    new_person: web::Json<CustomerDTO>,
    pool: web::Data<Pool>,
) -> Result<HttpResponse, ServiceError> {
...
pub async fn update(
    id: web::Path<i32>,
    updated_person: web::Json<CustomerDTO>,
    pool: web::Data<Pool>,
) -> Result<HttpResponse, ServiceError> {
...
mod tests {
    ...
    async fn get_people_in_db(pool: &web::Data<Pool>) -> Result<Vec<Customer>, String> {
        match address_book_service::find_all(pool) {
            Ok(data) => Ok(data),
            Err(err) => Err(format!("{:?}", err.error_response())),
        }
